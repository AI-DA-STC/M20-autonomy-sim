...
namespace qw {
    ...
    public:
        ...
        virtual void Run() {
            ...
            // Hip/knee: zero torques — let body settle naturally under gravity.
            // Wheels only: kd=2 to prevent free-spin above wheel_vel_limit (100 rad/s).
            // Hip/knee kd was removed because it causes instability on contact
            // with Gazebo ODE at low real-time factors.
            MatXf cmd = MatXf::Zero(16, 5);
            for (int i = 3; i < 16; i += 4) {
                cmd(i, 2) = 2.f;  // wheel kd only
            }
